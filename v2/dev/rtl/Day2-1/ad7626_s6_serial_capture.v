    ...
    if (FULL_CYCLE_CAPTURE != 0) begin : gen_full_cycle_capture
      ...
      always @(posedge dco_clk_s or negedge rstn) begin
        if (!rstn) begin
          ...
          // Why does this work
          // Since the new output of the upstream register will comes after a Clock-to-Q delay
          // And the downstream register can only get the old value of the upstream register at the edge.
          if (capture_arm_dco) begin            
            ...
            capture_arm_dco    <= 1'b0;        // '<=' new value available at the next cycle. 
          end else if (!capture_active_dco) begin
            if (!capture_req_seen_dco && capture_req_sys) begin
              shift_reg_dco          <= {SAMPLE_WIDTH{1'b0}};
              bit_count_dco          <= {BIT_COUNT_WIDTH{1'b0}};
              capture_req_seen_dco   <= 1'b1;
              capture_arm_dco        <= 1'b1;   // '<=' new value available at the next cycle.
